...
function Ticket(props) {
  ...
  console.log(Bus)
  // setTimeout(deleteData(Bus),120000);
  return (
    <View
      style={[
        globalShadowBox,
    ...
    >
      ...
      >
        ...
        <TouchableOpacity onPress={() => deleteData(Bus)}>
          <Bin />
        </TouchableOpacity>
      </View>
      ...
      <TouchableOpacity
        style={primaryButton}
        onPress={() =>
          navigation.navigate("TicketDetailScreen", { Bus, person })
        }
      >
        <Text
          style={{
...
function BookingPending({ navigation, route }) {
  ...
  function deleteBooking(Bus) {
    ...
    var NewPendingNode = rootReference.child(
